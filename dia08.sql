...
(1, 'Ana', 20, 'Engenharia', 8.5, 'Vitória'),
(2, 'Bruno', 22, 'Medicina', 9.0, 'Cachoeiro'),
(3, 'Carla', 19, 'Direito', 7.5, 'Vitória'),
(4, 'Diego', 21, 'Engenharia', 6.0, 'Cachoeiro'),
(5, 'Elisa', 20, 'Medicina', 9.5, 'Vila Velha');

-- 1. Mostrar todos os dados da tabela
SELECT * FROM alunos;

-- 2. Listar apenas nome e curso
SELECT nome, curso FROM alunos;

-- 3. Mostrar alunos do curso Engenharia
SELECT * FROM alunos
WHERE curso = 'Engenharia';

-- 4. Listar alunos com nota maior que 8.0
SELECT nome, nota_final FROM alunos
WHERE nota_final > 8.0;

-- 5. Mostrar os 3 alunos mais novos
SELECT * FROM alunos
ORDER BY idade ASC
LIMIT 3;

-- 6. Mostrar alunos de Medicina com nota acima de 9.0
SELECT * FROM alunos
